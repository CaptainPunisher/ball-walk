...
void screenCapture()
{
    ...
    if (fpo) {
        ...
        fclose(fpo);
        char s[256];
        sprintf(s, "convert ./vid/pic%03i.ppm ./vid/pic%03i.gif", 
                fnum, fnum);
        system(s);
        unlink(ts);
    }
    ...
    if (gl.walk || gl.keys[XK_Right] || gl.keys[XK_Left]) {
        ...
        for (int i=0; i<20; i++) {
            if (gl.keys[XK_Left]) {
                ...
                gl.camera[0] -= 2.0/lev.tilesize[0] * 
                    (0.05 / gl.delay);
                if (gl.camera[0] < 0.0)
                    gl.camera[0] = 0.0;
            } else {
...
void render(void)
{
    ...
    for (int j=0; j<ncols_to_render; j++) {
        ...
        for (int i=0; i<lev.nrows; i++) {
            if (lev.arr[row][col] == 'w') {
                ...
                glPushMatrix();
                Vec tr = { (Flt)j*dd+offx, 
                    (Flt)i*lev.ftsz[1]+offy, 0 };
    ...
    if (gl.exp44.onoff) {
        ...
        glAlphaFunc(GL_GREATER, 0.0f);
        glColor4ub(255,255,255,255);
        glTranslated(gl.exp44.pos[0], gl.exp44.pos[1], 
                gl.exp44.pos[2]);
